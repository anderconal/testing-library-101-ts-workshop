...
test('increments count on button click', async (t) => {
  // Arrange
  await t
    .expect(Selector(screen.getByText(/you clicked 0 times/i)).exists)
    .ok();
  ...
  // Assert
  await t
    .expect(Selector(screen.queryByText(/you clicked 0 times/i)).exists)
    .notOk();
  await t
    .expect(Selector(screen.getByText(/you clicked 1 times/i)).exists)
    .ok();
});
